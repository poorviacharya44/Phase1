...
public class AddProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/", "root", "Root@1999");
			Statement st=conn.createStatement();
			//select product database from before
			st.executeUpdate("use pet");
			int n=st.executeUpdate("insert into pets.product(name,color,price)values('"+name+"','"+color+"','"+priceval+"')");
			
			//sql query
			ResultSet rs = st.executeQuery("select * from pet.product");
